...
const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      try {
        // Attempt Restaurant login if admin login fails
        const RestaurantResponse = await axios.post("http://localhost:8080/Restaurant/login", { email, password });
        console.log("RestaurantResponse",RestaurantResponse)
        const { token, restaurantId, message } = RestaurantResponse.data;
        ...
        setTimeout(() => {
          navigate(`/SplashScreen?restaurantId=${restaurantId}`);
        }, 2000);
